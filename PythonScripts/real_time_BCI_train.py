...
def get_data(mode='train'):
    ...
    if windowed:
        ...
        if mode == 'test':
            if os.path.exists(subject_name + '_fb' + '_EEG_data.csv'):
                subject_name_i = subject_name + '_fb'
                print('Feedback path exists!')
            elif os.path.exists(subject_name + '_vrep_fb' + '_EEG_data.csv'):
                subject_name_i = subject_name + '_vrep_fb'
                print('VREP feedback path exists!')
        else:
            subject_name_i = subject_name

        data = np.genfromtxt(subject_name_i + '_EEG_data.csv', delimiter=',')
        labels = np.genfromtxt(subject_name_i + '_markers.csv', delimiter=',')
...
def init_globals_2(user_name='LameCap'):
    global subject_name

    subject_name = user_name


def train_network(expInfo, mode='train'):
    ...
    if clf_method == "LSTM":
        ...
        if mode == 'test':
            ...
            data_train_fb, label_train_fb, data_val_fb, label_val_fb = get_data(mode=mode)
            data_train = np.concatenate((data_train, data_train_fb), axis=0)
            data_val = np.concatenate((data_val, data_val_fb))
            label_train = np.concatenate((label_train, label_train_fb))
            label_val = np.concatenate((label_val, label_val_fb))
...
def save_data(feedback=False, vrep=False, markers=False, mode='train'):
    global final_data_array, data_ts, final_markers_array, markers_ts, init_ts_d, init_ts_m, subject_name

    if mode == 'space':
        subject_name = "D:/SpaceTrialRecordings092019/" + subject_name + "/" + "EEGData/"
    ...
    if mode == 'test':
        subject_name = subject_name + "_test"

    final_data_array = np.asarray(final_data)
    if mode == 'space':
        data_ts_array = np.asarray(data_ts)
    else:
        try:
    ...
    date_time = date_time.replace('.', '-')
    if mode == 'space':
        np.savetxt(subject_name + date_time + "_EEG_data.csv", final_data_array_ts, delimiter=",")
    else:
    ...
    if markers:
        ...
        if mode == 'space':
            markers_ts_array = np.asarray(markers_ts)
        else:
            try:
        ...
        if mode == 'space':
            np.savetxt(subject_name + date_time + "_markers.csv", final_markers_array_ts, delimiter=",")
        else:

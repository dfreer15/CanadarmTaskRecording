...
def calc_power(data_in_th, data_in_al, data_in_beta, ts):
    ...
    flpt_smoothed = EEG_p.smooth_plot(frontal_lobe_power_th)
    flpa_smoothed = EEG_p.smooth_plot(frontal_lobe_power_al)
    flpb_smoothed = EEG_p.smooth_plot(frontal_lobe_power_beta)
...
if __name__ == "__main__":
    ...
    # Process EEG data
    beta_p = PERd.create_beta_wave(eeg_path)
    beta_p = (beta_p - np.min(beta_p))/(np.max(beta_p)-np.min(beta_p))

    raw, ts0 = EEG_p.create_mne_raw_object(eeg_path)
    raw_np = EEG_p.clean_EEG(raw)
    segments, labels, ts = EEG_p.segment_signal_without_transition(raw_np, np.zeros(raw_np.shape[1]), 500, overlap=1)
    data = EEG_p.select_channels(segments)
    data = EEG_p.norm_dataset(data)

    mean_cov_n, conf_m = EEG_p.train_classifier(training_subs, test_sub, num_classes=5, win_size=500)
    Rdist = PERd.calc_distance_time(data, mean_cov_n)
    Rdist = (Rdist - np.min(Rdist))/(np.max(Rdist) - np.min(Rdist))
    ...
    ts = ts - gaze_ts_0"""
    markers_ar[:, 0] = markers_ar[:,0] - ts[0] - 5
    ts = ts - ts[0]
    ...
    plt.legend((p[0], p[1], p[2]), ('x', 'y', 'z'), loc='lower left')
    plot_markers()

    Rdist_out = EEG_p.smooth_plot(Rdist)
    beta_out = EEG_p.smooth_plot(beta_p)

    plt.subplot(412)
    axes = plt.gca()
    axes.set_ylim([0, 1])
    axes.set_xlim([ts[0], markers_ar[-1, 0]])
    p0 = plt.plot(ts[:-3], Rdist_out)
    p1 = plt.plot(ts[:-3], beta_out)
    plt.ylabel('EEG')
    plt.legend((p0[0], p1[0]), ('Rdist', 'beta'), loc='upper right')
    plot_markers()
